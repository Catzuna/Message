...
function updateDots() {
  ...
  dots.forEach((dot, idx) => {
    dot.style.background = idx < input.length ? "white" : "rgba(255,255,255,0.4)";
  });
}
...
function fireworksEffect() {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    function animate(timestamp) {
      ...
      const t = (timestamp - start) / 800;
      if (t > 1) {
        emoji.remove();
        return;
      }
      const x = 50 + radius * Math.cos(angle) * t;
      const y = 50 + radius * Math.sin(angle) * t - t * 30;
      emoji.style.left = `${x}%`;
      emoji.style.top = `${y}%`;
      emoji.style.transform = `translate(-50%, -50%) scale(${t})`;
